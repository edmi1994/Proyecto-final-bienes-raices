...
<div class="card mt-5">
    ...
    <b-form @submit.stop.prevent="handleSubmit(ingresar_propiedad)" >
    <div class="card-header">
        <label class="font-weight-bold">Insertar Propiedad</label>
            ...
            <b-col cols="4">
                <b-form-group label="Tipo">
                    <validation-provider name="Tipo" :rules="{ required: true}" v-slot="validationContext">
                        <b-form-select :state="getValidationState(validationContext)" v-model="propiedad.tipo" :options="tipos_modificados"></b-form-select>
                        <b-form-invalid-feedback id="input-2-live-feedback">{{ validationContext.errors[0] }}</b-form-invalid-feedback>
                    </validation-provider>
                </b-form-group>
            </b-col>
...
export default {
    ...
    methods:{
        ingresar_propiedad(){
            ...
            data.append('tipo_id', this.propiedad.tipo);
            data.append('url_imagen', this.propiedad.url_imagen);
            data.append('_method','POST');
            console.log(data);
            axios.post("/propiedades/store",data)
            .then(res => {    
